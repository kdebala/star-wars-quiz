...
export async function generateGameContent(mode: Mode) {
  const headerElText = gameHeaderText(mode);
  const modeRulesContentText = await gameDescriptionText(mode);
  headerEl.textContent = headerElText;
  modeRulesContent.textContent = modeRulesContentText;
}
